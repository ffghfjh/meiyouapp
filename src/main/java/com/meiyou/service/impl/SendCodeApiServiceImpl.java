...
public class SendCodeApiServiceImpl implements SendCodeApiService {
    ...
    //替换成你的AK
    final String accessKeyId = Constants.ACCESSKEYID;//你的accessKeyId,参考本文档步骤2
    final String accessKeySecret = Constants.ACCESSKEYSERECT;//你的accessKeySecret，参考本文档步骤2
    //初始化ascClient,暂时不支持多region（请勿修改）
    IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", accessKeyId,
            accessKeySecret);

    @Override
    public Msg sendRegistCode(String phone) {
        String verifyCode = String.valueOf(new Random().nextInt(899999) + 100000);//验证码
        String templatecode = "SMS_163675061"; //模板编码
        ...
        try {
            ...
            try {
                ...
                System.out.println("提示消息"+sendSmsResponse.getMessage());
    ...
    @Override
    public Msg sendRebackPwd(String phone) {
        String verifyCode = String.valueOf(new Random().nextInt(899999) + 100000);//验证码
        String templatecode = "SMS_163675060"; //模板编码
        String templateparam ="{\"code\":\""+verifyCode+"\"}";
        try {
            DefaultProfile.addEndpoint("cn-hangzhou", "cn-hangzhou", product, domain);
            IAcsClient acsClient = new DefaultAcsClient(profile);
